...
class SharedBuf():
    ...
    def __init__(self, desc):
        ...
        self._buf = multiprocessing.sharedctypes.RawArray(ctypes.c_char,
                                                          self._size)
    ...
    def put(self, value):
        ...
        try:
            ...
            # Always release lock. If we do not own it, we expect
            # an AssertionError that can be ignored
            try:
                self._cv.release()
    ...
    def get(self, next=True):
        try:
            # Only acquire the Condition object if we want to wait for the
            # next frame. There is no need to acquire it in other cases,
            # since the data reading is already properly locked by the
            # underlying multiprocessing Array object.
            if next:
                self._cv.acquire()
                self._cv.wait()
            v = self._desc()
